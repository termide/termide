...
impl App {
    ...
    pub(super) fn handle_modal_key(&mut self, key: crossterm::event::KeyEvent) -> Result<()> {
        ...
        if let Some(modal) = self.state.get_active_modal_mut() {
            ...
            let modal_result = match modal {
                ActiveModal::Confirm(m) => {
                    m.handle_key(key)?.map(|r| match r {
                        ModalResult::Confirmed(value) => ModalResult::Confirmed(Box::new(value) as Box<dyn std::any::Any>),
            ...
            if let Some(result) = modal_result {
                ...
                if matches!(modal, ActiveModal::RenamePattern(_)) && matches!(result, ModalResult::Cancelled) {
                    // Take operation from pending action and return to ConflictModal
                    if let Some(action) = self.state.take_pending_action() {
                        if let crate::state::PendingAction::RenameWithPattern { operation, .. } = action {
                            ...
                            if let Some(source) = operation.current_source() {
                                ...
                                self.state.pending_action = Some(crate::state::PendingAction::ContinueBatchOperation {
                                    operation,
                                });
    ...
    pub(super) fn handle_modal_result(&mut self, value: Box<dyn std::any::Any>) -> Result<()> {
        if let Some(action) = self.state.take_pending_action() {
            match action {
                crate::state::PendingAction::CreateFile { panel_index, directory } => {
                    self.handle_create_file(panel_index, directory, value)?;
                }
                crate::state::PendingAction::CreateDirectory { panel_index, directory } => {
                ...
                crate::state::PendingAction::SaveFileAs { panel_index, directory } => {
                ...
                crate::state::PendingAction::OverwriteDecision { panel_index, source, destination, is_move } => {
                    self.handle_overwrite_decision(panel_index, source, destination, is_move, value)?;
                }
                crate::state::PendingAction::CopyPath { panel_index, sources, target_directory } => {
                    self.handle_copy_path(panel_index, sources, target_directory, value)?;
                }
                crate::state::PendingAction::MovePath { panel_index, sources, target_directory } => {
                ...
                crate::state::PendingAction::RenameWithPattern { operation, original_name } => {
